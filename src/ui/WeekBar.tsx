...
interface Props { plan: PlannedMeal[]; recipes: Recipe[]; servings:number; wanted:number; onServings:(n:number)=>void; onWanted:(n:number)=>void; onSurprise:()=>void; onClear:()=>void; onRemove:(key:string)=>void; }
export function WeekBar({plan,recipes,servings,wanted,onServings,onWanted,onSurprise,onClear,onRemove}:Props) {
 return <section className="fresh-week" aria-label="This week">
   ...
   <div className="fresh-selection"><div className="fresh-thumbs">
     {plan.map(meal=><button key={meal.key} className="selected-meal" aria-label={`Remove ${recipes.find(r=>r.id===meal.recipeId)?.name || 'meal'}`} onClick={()=>onRemove(meal.key)}><MealPhoto id={meal.recipeId}/><span><Icon name="check"/></span></button>)}
     {Array.from({length:Math.min(5,Math.max(0,wanted-plan.length))},(_,i)=><a href="#choose-meals" className="empty-meal" key={i} aria-label="Choose another dinner"><Icon name="plus"/></a>)}
   </div><p aria-live="polite"><strong>{plan.length} of {wanted}</strong><span>dinners picked</span></p></div>
