...
/**
 * Hand this browser's Tesco session to Supermarket running on this computer.
 *
 * This is the phone's only route in: a phone has no Tesco session and no
 * extension, so the copy running on this computer needs one, and this is where
 * it comes from. The app in a tab does not use any of it.
 *
 * It used to work by reloading a Tesco page and hoping a request went past
 * while we were watching. Now it simply reads the token, which is the same
 * thing without the hoping.
 */
async function handOver() {
  await grabToken();
  const { tescoHeaders: headers } = await chrome.storage.session.get('tescoHeaders');
  if (!headers?.authorization) {
    await chrome.storage.session.set({ status: 'Could not read your Tesco session. Open tesco.com, sign in, and try again.' });
    return false;
  }
  ...
  const { nonce: held } = await chrome.storage.session.get('nonce');
  let nonce = held ?? (await freshNonce());
...
chrome.runtime.onMessage.addListener((message, sender, reply) => {
  ...
  if (message.type === 'keep') {
    (async () => {
      ...
      reply({ ok: true });
      if (message.on) void handOver().catch(() => chrome.storage.session.set({
  ...
  (async () => {
    try {
      ...
      if (!nonce) {
        await chrome.storage.session.set({ status: 'Supermarket is not running on this computer. Start it, then try again.' });
        reply({ ok: false });
        return;
      }
      await chrome.storage.session.set({ nonce, status: 'Reading your Tesco session…' });
      reply({ ok: true });
      await handOver();
    } catch {
      await chrome.storage.session.set({ status: 'Supermarket is not running on this computer. Start it, then try again.' });
...
chrome.webRequest.onBeforeSendHeaders.addListener(
  (details) => { void capture(details).catch(() => {}); },
...
/**
 * A free top-up while you browse.
 *
 * Reading the page covers the case where nobody has touched Tesco recently.
 * This covers the easier one: a Tesco tab in use makes requests carrying a live
 * token and a customer-uuid, which costs nothing to notice. Neither replaces
 * the other, and nothing here is written to disk.
 */
async function capture(details) {
  ...
  const { tescoHeaders = {} } = await chrome.storage.session.get('tescoHeaders');
  await chrome.storage.session.set({ tescoHeaders: { ...tescoHeaders, ...headers } });
  await note({ requests: 'yes', token: 'captured from a Tesco page' });
}
